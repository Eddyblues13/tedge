...
<div class="main-content">
    <div class="container-fluid" style="max-width:900px;">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
            <div>
                <h4 class="admin-page-title mb-1">Add New Trader</h4>
                <p class="admin-page-subtitle mb-0">Fill in the details to add a new expert trader</p>
            </div>
        ...
        <form id="addTraderForm" action="{{ route('traders.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="ct-card ct-card-photo mb-4">
                ...
                <div class="ct-photo-zone" id="photoDropZone">
                    <input type="file" id="addPictureInput" name="picture" accept="image/*" class="d-none" required>
                    ...
                    <div id="photoPreviewState" class="ct-photo-preview-state" style="display:none;">
                        <div class="ct-avatar-ring">
                            <img id="addPreviewImg" src="" class="ct-avatar-img">
                            <div class="ct-avatar-change"><i class="bi bi-pencil-fill"></i></div>
                        </div>
                        <div class="ct-file-meta">
                            <span id="fileName" class="ct-file-name"></span>
                            <span id="fileSize" class="ct-file-size"></span>
                        </div>
                        ...
                        <div class="ct-field">
                            ...
                            <div class="ct-input-wrap">
                                ...
                                <input class="ct-input" placeholder="e.g. 15.5" type="number" step="any"
                                    name="return_rate" value="{{ old('return_rate', 0) }}" min="0" required>
                        ...
                        <div class="ct-field">
                            ...
                            <div class="ct-input-wrap">
                                ...
                                <input class="ct-input" placeholder="e.g. 20" type="number" step="any"
                                    name="profit_share" value="{{ old('profit_share', 0) }}" min="0" required>
                                <span class="ct-input-suffix">%</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="ct-field">
                            ...
                            <div class="ct-input-wrap">
                                ...
                                <input class="ct-input ct-input-has-prefix" placeholder="e.g. 100" type="number"
                                    step="any" name="min_amount" value="{{ old('min_amount') }}" min="0" required>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="ct-field">
                            ...
                            <div class="ct-input-wrap">
                                ...
                                <input class="ct-input ct-input-has-prefix" placeholder="e.g. 50000" type="number"
                                    step="any" name="max_amount" value="{{ old('max_amount') }}" min="0" required>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div id="uploadOverlay" class="ct-upload-overlay" style="display:none;">
                <div class="ct-upload-card">
                    ...
                    <p class="ct-upload-label" id="uploadLabel">Uploading trader...</p>
            ...
            <div class="d-flex gap-3 mb-5">
                <button type="submit" class="ct-submit-btn" id="addTraderBtn">
                    <i class="bi bi-plus-circle me-2"></i> Add Trader
                </button>
...
{{-- SVG defs for gradient (must be in body) --}}
...
<script>
    ...
    const fileInput    = document.getElementById('addPictureInput');
    ...
    form.addEventListener('submit', function(e) {
        ...
        setProgress(0);
        uploadLabel.textContent = 'Uploading image...';
        ...
        xhr.upload.addEventListener('progress', function(ev) {
            if (ev.lengthComputable) {
                ...
                const mapped = Math.round(uploadPct * 50); // 0-50%
                currentProgress = mapped;
                setProgress(currentProgress);
                if (uploadPct >= 1) {
                    // Upload to server done, start phase 2
        ...
        // Phase 2: Animated progress 50% → 95% while server processes (Cloudinary upload etc.)
            ...
            animateTo100(function() {
                try {
                    ...
                    if (xhr.status >= 200 && xhr.status < 300) {
                        uploadLabel.textContent = 'Trader added successfully!';
                        if (typeof toastr !== 'undefined') toastr.success(data.message || 'Trader added!');
                        setTimeout(() => {
                        ...
                        if (data.errors) {
                            ...
                            if (typeof toastr !== 'undefined') toastr.error(data.message || 'Error adding trader');
                        }
                    }
                } catch {
                    overlay.style.display = 'none';
        ...
        xhr.open('POST', "{{ route('traders.store') }}");
